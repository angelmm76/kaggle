...
for pr in targetprods:
    ...
    xgtrain = xgboost.DMatrix(Xtrain, label=ytrain.loc[:, pr])
    xgval = xgboost.DMatrix(Xval, label=yval.loc[:, pr])
    evals = [(xgval,'eval'), (xgtrain,'train')]
    clf = xgboost.train(plst, xgtrain, num_rounds, evals)	
    clfdict[pr] = clf
    #ypredv = clf.predict(Xval)
    #probs.append(clf.predict_proba(Xval)[:, 1])
    probs.append(clf.predict(xgboost.DMatrix(Xval)))
...
#tp0, Xtest[:500000]
#tp01, Xtest[500000:929615]
#testprobs = np.vstack((tp0, tp1))
...
# Submit 
print('Submitting...')
subname = 'Data/SantanderXGB10_lag4_14pr_nocv.csv'
sub = tids.to_frame()
