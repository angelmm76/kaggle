...
def getClientFeatures(mdata, prmdata):
    ...
    Xclient = pd.concat([mdata[['ncodpers', 'employeeYN', 'sexo', 'age',
                                ...
                                #pd.get_dummies(mdata['cod_prov']), # 52
...
print('Extracting data from several months')
X_list = []
y_list= []
train_months = ['2015-06-28', '2015-05-28']
print(train_months)
for thismonth in train_months:
    # Client features
    ...
    X = getClientFeatures(mdata[clientfeatures], prevmdata).drop(['ncodpers'], 
                                                                        axis=1)
    ...
    addedprods= np.sum(y, axis=1)
    Xtrain_m = X[addedprods!=0]
    ytrain_m = y[addedprods!=0]
    print(Xtrain_m.shape, ytrain_m.shape)
    del X, y
    gc.collect()
    X_list.append(Xtrain_m)
    y_list.append(ytrain_m)
    del Xtrain_m, ytrain_m
    gc.collect()
    
Xtrain = pd.concat(X_list, ignore_index=True)
ytrain = pd.concat(y_list, ignore_index=True)
del X_list, y_list
gc.collect()

# Test data has no R feature
if 'R' in Xtrain.columns:
    Xtrain.drop(['R'], axis=1, inplace=True)
    print('tiprel value R removed from Xtrain')
    
# Fix to remove feature_names mismatch in Xtest prediction
...
for pr in targetprods:
    ...
    eval_set = [(Xval, yval.loc[:, pr])]
    #clf.fit(Xtrain, ytrain.loc[:, pr], eval_set= eval_set,
...
# XGB, 12 prods, onlyadded, jun15 + may16: 0.82264 (LB 0.0135988)
# XGB, 12 prods, onlyadded, jun15 + may15: 0.82605 (LB 0.0265798)
# XGB, 22 prods, onlyadded, jun15 + may15: 0.79629 (LB  0.0264526)
# XGB, 12 prods, onlyadded, jun15+may15+abr15: 0.82853 (LB 0.0258663)
# XGB, 12 prods, onlyadded, jun15+may15+abr15+mar15: 0.83235 (LB 0.0255401)

# jun2015 all clients, 22 prods  (prod05, XGB:0.04621)
# less: XGB: 0.04688 (LB 0.0270346),   RF: 0.04503   BernNB: 0.04300
# XGB+eval_set: 0.79219??? (LB 0.0270533)
# jun2015 onlyaddedclients, 12 prods  (prod05, XGB:0.04621)
# XGB: 0.04688 (LB 0.0270346),   RF: 0.04503   BernNB: 0.04300

# Prod07
# jun2015 100000 clients, 12 prods of jun15
#     XGB, 0.04493 (LB 0.0268196) RF: 0.04377   BernNB: 0.04197

# cod prov jun2015 100000 clients, 12 prods of may16
#     XGB, 0.04472 (LB 0.0270201) RF: 0.04202  BernNB: 0.04151

# cod prov jun2015 all clients, 12 prods of may16
#     XGB, 0.04636 (LB 0.0271445) RF: 0.04290  BernNB: 0.04281
...
# Submit 
print('Submitting...')
subname = 'Data/SantanderXGB9_onlyadded_2mo.csv'
sub = tids.to_frame()
