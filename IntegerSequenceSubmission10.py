    ...
    for i in range(len(X)):
        ...
        if n == 0:
            ...
            if np.mean(X[i]==mode) > threshold or n < numberPoints :  # mode frequency
                ...
                a = np.arange(numberPoints).reshape((-1,1))
                poly = PolynomialFeatures(degree) # d degree polynomial
                a = poly.fit_transform(a)
                b = np.array(X[i][-numberPoints:])
                #print((X[i], b, a, b.shape, a.shape))
                ridge.fit(a, b)
                #pred = ridge.predict(poly.transform(numberPoints)).astype(np.int64)
                #ypred.append(pred[0])
                if ridge.score(a,b) > 0.5:
                    pred = ridge.predict(poly.transform(numberPoints)).astype(np.int64)
                    ypred.append(pred[0])
                    ypredarray[i] = pred
                    scores.append(ridge.score(a,b))
                    if ytrain[i]==pred: fitpred += 1
                else:
                    # Difference seq
                    difSeq = np.array([X[i][-j-1]-X[i][-j-2] 
                                    for j in range(numberPoints-1)])
                    ## Ridge fit
                    ridge = Ridge(alpha=alpha)
                    a = np.arange(numberPoints-1).reshape((-1,1))
                    #poly = PolynomialFeatures(degree) # d degree polynomial
                    #a = poly.fit_transform(a)
                    #print((X[i], b, a, b.shape, a.shape))
                    ridge.fit(a, difSeq)
                    pred = X[i][-1] + \
                                ridge.predict(numberPoints-1).astype(np.int64)
                            #(poly.transform(numberPoints-1)).astype(np.int64)
                    ypred.append(pred[0])
                    ypredarray[i] = pred
...
def main():
    ...
    # Saving submission file
    testpred = predTarget(list_test, ytrain,  alpha, thres, degree, npoints)
    print('testpred: ' + str(testpred[:10]))
    predictions = [[tidx[i], testpred[i]] for i in range(testpred.size)]

    np.savetxt(currentdir + '\\Data\\submission8.csv', predictions,
                delimiter=',', fmt='%d,"%s"', 
