...
def getValidation(cfr, train, target):
    # Stratified Fold
    cv = cross_validation.StratifiedKFold(target, n_folds=3, shuffle=True)
...
def main():
    ...
    # Names
    tfv = TfidfVectorizer(min_df=2, ngram_range=(1,2), sublinear_tf = True)
    tfv.fit(train_df['Name'])
    vfeat = tfv.transform(train_df['Name'])
    print('Tfid vect: ' + str(vfeat.shape))
    svd = TruncatedSVD(n_components = 100, n_iter= 5)
    svd.fit(vfeat)
    vfeat = svd.transform(vfeat)
    print('Svd Tfid vect: ' + str(vfeat.shape))
    ...
    vfeat_test = tfv.transform(test_df['Name'])
    vfeat_test = svd.transform(vfeat_test)
    ...
    # Add text features
    test_data = np.hstack((test_data, vfeat_test))
    ...
    #print('\nValidation for Selecting params of Random Forest with GridCV...')
    ...
    #grid = GridSearchCV(rf, param_grid, cv=5, n_jobs=2)
    #grid.fit(Xtrain, ytrain)
    #print('Best score: ' + str(grid.best_score_))
    #print('Best parameters:' + str(grid.best_params_))
    #bestforest = grid.best_estimator_
    #getValidation(bestforest, Xtrain, ytrain)
    ...
    ## with test features {'C': 10, 'gamma': 0.001, 'class_weight': None}
    ...
    predictions_file = open("TitanicSurvivalSubmission4.csv", "wb")
